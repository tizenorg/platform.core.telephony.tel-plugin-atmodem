	...
	{		
		...
		if((sw1 == 0x90 && sw2 == 0x00) || sw1 == 0x91)	{
			...
			switch (file_meta->file_id) 
			{
					...
					case SIM_EF_SST:
/*					if(tcore_sim_get_type(o) == SIM_TYPE_GSM)
							dr = tcore_sim_decode_sst(&file_meta->files.data , fileData, response_len);
						else if(tcore_sim_get_type(o) == SIM_TYPE_USIM)
							dr = tcore_sim_decode_ust(&po->st_u.ust, fileData, response_len);
						else {
					...
					case SIM_EF_MBDN:
/*					dr = tcore_sim_decode_xdn(&file_meta->files.data.mailbox, fileData, strlen(fileData));*/
						break;
					...
					case SIM_EF_USIM_MBI:
/*					dr = tcore_sim_decode_mbi(&po->mb_u.mailbox.mbi, fileData, strlen(fileData));*/
						break;
					...
					case SIM_EF_CPHS_CALL_FORWARD_FLAGS:
						dr = tcore_sim_decode_cff(&file_meta->files.data.cf, (unsigned char *)fileData, strlen(fileData));
						break;

					case SIM_EF_CPHS_CUSTOMER_SERVICE_PROFILE:
/*					dr = tcore_sim_decode_csp(&po->p_cphs->csp, fileData, strlen(fileData));*/
						break;

					case SIM_EF_CPHS_VOICE_MSG_WAITING:
						dr = tcore_sim_decode_vmwf(&file_meta->files.data.mw.mw_data_u.cphs_mw, (unsigned char *)fileData, strlen(fileData));
						break;

					case SIM_EF_CPHS_MAILBOX_NUMBERS:
/*					dbg("[CPHS]decoding SIM_EF_CPHS_MAILBOX_NUMBERS");
						if (file_meta->current_index == 1)
							dr = tcore_sim_decode_xdn(&po->mb_u.mbn.voice_line1, fileData, strlen(fileData));
						else if (file_meta->current_index == 2)
							dr = tcore_sim_decode_xdn(&po->mb_u.mbn.voice_line2, fileData, strlen(fileData));
						else if (file_meta->current_index == 3)
							dr = tcore_sim_decode_xdn(&po->mb_u.mbn.video, fileData, strlen(fileData));
						else if (file_meta->current_index == 4)
							dr = tcore_sim_decode_xdn(&po->mb_u.mbn.fax, fileData, strlen(fileData));
						else {
					...
					case SIM_EF_USIM_MWIS:
						dr = tcore_sim_decode_mwis(&file_meta->files.data.mw.mw_data_u.mw, (unsigned char *)fileData, strlen(fileData));
						break;

					case SIM_EF_USIM_CFIS:
						dr = tcore_sim_decode_cfis(&file_meta->files.data.cf, (unsigned char *)fileData, strlen(fileData));
						break;
					...
					case SIM_EF_CPHS_DYNAMICFLAGS:
/*					dr = tcore_sim_decode_dynamic_flag(&po->p_cphs->dflagsinfo, fileData, strlen(fileData));*/
						break;
...
/*					dr = tcore_sim_decode_dynamic2_flag(&po->p_cphs->d2flagsinfo, fileData, 	strlen(fileData));*/
					...
					case SIM_EF_CPHS_INFORMATION_NUMBERS:
/*					dr = tcore_sim_decode_information_number(&po->p_cphs->infn, fileData, strlen(fileData));*/
						break;
